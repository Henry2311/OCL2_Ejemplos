...
def procesar_declaracion(instr, ts):
    ...
    simbolo = Simbolos(id, TIPO_DATO.ENTERO, id)
    ts.agregar(simbolo)
    ts.dato += f'{id}: .word 0\n'
...
def procesar_while(instr, ts):
    ...
    ts.salida += f'{ciclo}:\n' #WHILE 
    expLog = resolver_expresion_logica(instr.expLogica, ts) #SI VERDADERO EJECUTA
...
def resolver_expresion(expCad, ts) :
    if isinstance(expCad, ExpresionBinaria) :
        ...
        # a ->  temporal
        temporal = ts.generateTemporal()
        ts.salida += f'''la t{temporal}, {expCad.id}\n'''
...
def resolver_expresion_aritmetica(expNum, ts) :
    if isinstance(expNum, ExpresionBinaria) :
        ...
        if expNum.operador == OPERACION_ARITMETICA.MENOS : 
            temporal = f't{ts.generateTemporal()}'
            ts.salida += f'sub {temporal}, {exp2}, {exp1}\n'
            return temporal
        if expNum.operador == OPERACION_ARITMETICA.POR : 
            temporal = f't{ts.generateTemporal()}'
            ts.salida += f'mul {temporal}, {exp1[0]}, {exp2}\n'
            return temporal
        ...
        exp = resolver_expresion_logica(expNum, ts)
        return exp
    elif isinstance(expNum, ExpresionID):
        temporal = ts.generateTemporal()
        ts.salida += f'''la t{temporal}, {expNum.id}\n'''
...
def resolver_expresion_logica(expLog, ts):
    ...
    if expLog.operador == OPERACION_LOGICA.MAYOR_QUE : 
        ...
        ts.salida += f'bgt {exp1[0]}, {exp2}, {etiqueta}\n'
        return etiqueta
    ...
    '''
    if 10<11, salto a Validacion2 listo
    salto a salida listo
    validacion2: listo
    if 10<9, salto a ejecucion
    salto a salida
    ejecucion
    '''
...
def procesar_instrucciones(instrucciones, ts, save=False) :
    for instr in instrucciones :
        if not save:
            ...
            elif isinstance(instr, While): return procesar_while(instr, ts)
