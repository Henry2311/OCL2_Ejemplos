...
InstrFunction = "void" _ id:([a-zA-Z0-9_]+) _ "("_ params:ListaParams? _")"_"{" _ instrucciones:Instrucciones _ "}" {
    return new InstrFunction(id.join(""),params, instrucciones);
}

ListaParams = head:Param _ "," _ tail:ListaParams {
    return [head].concat(tail)
}
            / param:Param {return [param]}

Param = "int" _ param:([a-zA-Z0-9_]+) {return param.join("")}

CallFunction = id:([a-zA-Z0-9_]+) _ "("_ params:ListaParamsCall? _")"_";" {
    return new InstrFunction(id.join(""), params);
}

ListaParamsCall = head:Op_Aritmeticas _ "," _ tail:ListaParamsCall {
    return [head].concat(tail) }
                / param:Op_Aritmeticas {return [param]}
