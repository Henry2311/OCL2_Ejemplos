...
public class CompilerVisitor : LanguageBaseVisitor<object>
{
    ...
    public override object VisitArryDclStmt([NotNull] LanguageParser.ArryDclStmtContext context)
    {
        string id = context.arrayDcl().ID().GetText();
        string typeText = context.arrayDcl().type().GetText();

        var list_values_array =  context.arrayDcl().list_values()?.expr().Select(expr => (int) Visit(expr)).ToList() ?? new List<int>();
        
        currentEnvironment.SetVariable(id,list_values_array, SymbolType.Integer);

        return null;
    }

    public override object VisitSliceDclStmt([NotNull] LanguageParser.SliceDclStmtContext context)
    {
        string id = context.sliceDcl().ID().GetText();
        string typeText = context.sliceDcl().type().GetText();

        currentEnvironment.SetVariable(id,new List<int>(), SymbolType.Integer);

        return null;
    }

    public override object VisitAppendSliceStmt([NotNull] LanguageParser.AppendSliceStmtContext context)
    {
        string id = context.appendSlice().ID().GetText(); //ID
        var array_values = (List<int>) currentEnvironment.GetVariable(id).Value; //ARRAY DINAMICO
        var value = Visit(context.appendSlice().expr()); //VALOR A AGREGAR

        array_values.Add((int) value);

        //currentEnvironment.SetVariable(id, array_values, SymbolType.Integer);

        return null;
    }

    public override object VisitArrayAccessExpr([NotNull] LanguageParser.ArrayAccessExprContext context)
    {
        string id = context.arrAccess().ID().GetText();
        var index = Visit(context.arrAccess().expr());

        var array_values = (List<int>) currentEnvironment.GetVariable(id).Value;
        
        return array_values[(int) index];
    }
